...
export function generateIssuerKeys() {
    ...
    if (!fs.existsSync(config.get<PathLike>('PATH_TO_PRIVATE_KEY')) || !fs.existsSync(config.get<PathLike>('PATH_TO_PUBLIC_KEY'))) {
        const { publicKey, privateKey } = crypto.generateKeyPairSync('rsa', {
            modulusLength: 2048,
        });
        // Save the keys to files
        fs.writeFileSync(config.get<PathOrFileDescriptor>('PATH_TO_PRIVATE_KEY'), privateKey.export({ type: 'pkcs1', format: 'pem' }));
        fs.writeFileSync(config.get<PathOrFileDescriptor>('PATH_TO_PUBLIC_KEY'), publicKey.export({ type: 'spki', format: 'pem' }));
...
export function getPrivateKey(): number[] {
    const keyPem = fs.readFileSync(config.get<PathOrFileDescriptor>('PATH_TO_PRIVATE_KEY'), 'utf-8');
...
export function getPublicKey(): number[] {
    const keyPem = fs.readFileSync(config.get<PathOrFileDescriptor>('PATH_TO_PUBLIC_KEY'), 'utf-8');
