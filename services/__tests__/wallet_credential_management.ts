...
import { Data } from "../identity_holder/src/interface"
...
describe('Wallet V2: Management of Credentials', () => {
    ...
    test('Test v2/credentials works', ()=> {
        ...
        const all_credentials: string[] = getCredentialsV2(user).body.credentials
    ...
    test('Test v2/credential works', ()=> {
        ...
        const credential = getCredentialV2(user, credential1).body
    ...
    test('Test v2/credential works for second test credential', ()=> {
        ...
        const credential = getCredentialV2(user, credential2).body
        expect(credential).toStrictEqual({
    ...
    test('Test v2/credential fails for invalid credential id', ()=> {
        ...
        expect(getCredentialV2(user, credential1).status).toBe(400)
    })
    ...
    test('Test v2/credential can delete credential', ()=> {
        ...
        const user = data.users[0]
        const credential1 = "e8b08949-8369-48d2-9d6f-2a1c2f1d52ec"
        expect(getCredentialV2(user, credential1).status).toBe(200)
        expect(getCredentialsV2(user).body.credentials.length).toBe(2)
        expect(deleteCredentialV2(user, credential1).status).toBe(200)
        expect(getCredentialV2(user, credential1).status).toBe(400)
        expect(getCredentialsV2(user).body.credentials.length).toBe(1)
    })
    ...
    test('Test v2/credential fails to delete invalid credential', ()=> {
        ...
        expect(getCredentialsV2(user).body.credentials.length).toBe(1) // For some reason state isnt being reset between the tests. Going with it for now.
        expect(deleteCredentialV2(user, credential1).status).toBe(400)
        expect(getCredentialsV2(user).body.credentials.length).toBe(1)
    })
})
